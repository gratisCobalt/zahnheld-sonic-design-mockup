...
const EssentialCard = ({ item, index }: { item: EssentialItem; index: number }) => {
  ...
  return (
    ...
    >
      <div
        ref={cardRef}
        className="relative overflow-hidden aspect-[3/4] mb-6 rounded-2xl cursor-pointer group"
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6">
          <span
            className="section-label mb-2 block transition-colors duration-500"
            style={{ color: 'rgba(255,255,255,0.4)' }}
          >
            {item.number}
          </span>
          <h3 className="text-xl font-bold uppercase tracking-tight text-white mb-1">{item.title}</h3>
...
const Essentials: React.FC = () => {
  return (
    <section className="bg-[#050505] text-white py-32 md:py-48">
      <div className="max-w-[1200px] mx-auto px-8">
        <div className="flex justify-between items-end mb-16">
          <div>
            <span className="section-label text-white/30 block mb-2">THE ESSENTIALS</span>
          </div>
          <a href="#" className="section-label text-white/50 hover:text-white transition-colors cursor-pointer">
            VIEW ALL
          </a>
        </div>
        ...
        <motion.p
          ...
          viewport={{ once: true }}
          className="text-center text-white/20 text-lg md:text-xl italic font-serif mb-16"
        >
          "Every detail, essential."
        </motion.p>
