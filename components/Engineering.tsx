...
const PartLabel: React.FC<{ 
  ...
  opacity: any;
  delay?: number;
}> = ({ title, specs, side, yOffset, opacity, delay = 0 }) => {
  return (
    <motion.div 
...
const Engineering: React.FC = () => {
  ...
  // Adjusted offset to start earlier and last longer
  ...
  // EXPLOSION LOGIC
  // Range: 0% to 40% of scroll triggers the full explosion
  const range = [0, 0.4];

  // --- ADJUSTED VALUES FOR VIEWPORT SAFETY ---
  
  // 1. Brush Head - Reduced from -320% to -200% to stay below header
  const headY = useTransform(scrollYProgress, range, ["0%", "-200%"]);
  
  // 2. Shaft/Neck - Reduced proportionaly
  const shaftY = useTransform(scrollYProgress, range, ["0%", "-140%"]);
  
  // 3. Housing - Stays roughly central
  const housingOpacity = useTransform(scrollYProgress, [0, 0.2], [1, 0]); 
  const housingScale = useTransform(scrollYProgress, range, [1, 1.05]);

  // 4. Motor - Slight upward move
  const motorY = useTransform(scrollYProgress, range, ["0%", "-60%"]);
  
  // 5. Battery - Moves Down (Anchor)
  const batteryY = useTransform(scrollYProgress, range, ["0%", "50%"]);
  
  // 6. PCB - Moves Side
  const pcbX = useTransform(scrollYProgress, range, ["0%", "160%"]); 
  const pcbOpacity = useTransform(scrollYProgress, [0, 0.2], [0, 1]); 

  // 7. Charging Coil - Reduced downward travel to keep in view
  const coilY = useTransform(scrollYProgress, range, ["0%", "180%"]);

  // Global Opacity for labels
  ...
  return (
    <section ref={targetRef} className="relative h-[400vh] bg-[#050505] z-30">
      ...
      <div className="sticky top-0 h-screen w-full flex flex-col items-center justify-center overflow-hidden">
        ...
        {/* Added translate-y-16 to shift center of gravity down, giving more room for top explosion */}
        <div className="relative w-[280px] md:w-[320px] h-[600px] flex items-center justify-center z-10 perspective-1000 translate-y-16 md:translate-y-24">
          ...
          {/* A. BRUSH HEAD */}
          <motion.div style={{ y: headY }} className="absolute top-[10%] z-50 flex flex-col items-center w-full">
          ...
          {/* B. SHAFT / METAL NECK */}
          <motion.div style={{ y: shaftY }} className="absolute top-[22%] z-40 flex flex-col items-center w-full">
            <PartLabel title="Vibration Shaft" specs="Stainless Steel 316L" side="right" yOffset="20px" opacity={labelOpacity} />
          ...
          {/* D. MOTOR */}
          <motion.div style={{ y: motorY }} className="absolute top-[32%] z-20 flex flex-col items-center w-full">
             <PartLabel title="Maglev Engine" specs="60,000 Movements/Min" side="left" yOffset="40px" opacity={labelOpacity} />
          ...
          {/* E. BATTERY & CHASSIS */}
          <motion.div style={{ y: batteryY }} className="absolute top-[48%] z-10 flex flex-col items-center w-full">
             <PartLabel title="Energy Core" specs="Li-Ion 14500 / 800mAh" side="left" yOffset="80px" opacity={labelOpacity} />
          ...
          {/* F. MAIN PCB */}
          <motion.div style={{ x: pcbX, opacity: pcbOpacity }} className="absolute top-[48%] z-15 w-[36px] h-[140px] bg-[#0d2e16] border border-[#26a648]/30 rounded shadow-xl flex flex-col items-center py-2 backdrop-blur-md">
